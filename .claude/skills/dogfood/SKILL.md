...
Exercise plugin-bot by making it do its job: build a real plugin capability in the `plugins/dogfood/claude-code` sandbox, then grade both the artifact and the guidance system that produced it. This is a repo-local skill (like effected's `improve`): it may assume this repo's layout, which shipped plugins never can.
...
**1. Sandbox clean.** `plugins/dogfood/claude-code` must contain only `.claude-plugin/plugin.json`. If a prior cycle left components behind, ask the user whether to reset before building.

**2. Dispatch the build.** Send the task to the `plugin-bot:plugin-engineer` agent: build, inside `plugins/dogfood/claude-code`, a plugin that does what $ARGUMENTS describes, using the `plugin-setup` skill for scaffolding. Do not build it yourself in the main context — the point is exercising the agent, its preloads and the path enforcers. While it works, note which skills fire and which references it reads.

**3. Validate the artifact.** `claude plugin validate plugins/dogfood/claude-code --strict` must pass before anything else counts.
...
**7. Reset the sandbox.** With user confirmation, remove everything under `plugins/dogfood/claude-code` except `.claude-plugin/plugin.json`, so the next cycle starts clean.

## Rules

- The sandbox is disposable; plugin-bot is not. Nothing in this cycle edits `plugins/plugin-bot/claude-code` — that separation is what makes the harvest honest.

...
- **There is no universal plugin-root placeholder.** `${CLAUDE_PLUGIN_ROOT}` is Claude's, `${PLUGIN_ROOT}` is Agent Plugins' and Copilot's; Copilot is the only host that answers to both. `${COPILOT_PLUGIN_ROOT}` is defined by nothing — if you see it, it is a bug.

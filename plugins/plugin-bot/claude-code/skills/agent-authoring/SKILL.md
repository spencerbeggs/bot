...
- **A malformed frontmatter still loads the agent**, falling back to the filename and a generic description — which is why a broken agent fails quietly rather than loudly. Check the file parses as valid YAML before trusting that its `description` is what's actually driving delegation.

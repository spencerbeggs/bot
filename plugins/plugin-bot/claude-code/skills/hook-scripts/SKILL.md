...
2. **`set -euo pipefail`** present; fails open when `jq` is missing — a hook that cannot parse its envelope must not block the user. Fail open means `emit_noop; exit 0`, never `exit 2` and never a bare non-zero.

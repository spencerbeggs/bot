---
name: monitors
description: Read before creating or editing any monitors/monitors.json entry or any watch script under monitors/ (.ts, .mts, .js, .mjs). Nothing loads this skill automatically on this host, so reach for it yourself the moment one of those files comes into scope. Use when authoring, auditing, or registering a Claude Code plugin monitor: monitors are a Claude Code feature, and this skill is the authority for building one no matter which host you are authoring from. Ships the typed TypeScript poll-monitor harness (self-scheduling loop, stable-streak debounce, notify-once dedup, --once mode) plus the registration rules and notification-copy doctrine.
---

# Monitors

House style for plugin monitors: background processes Claude Code starts automatically while the plugin is active, whose every stdout line lands in the session as a notification. Monitors are experimental and need Claude Code v2.1.105+; the platform contract (fields, `when:` values, substitution variables, constraints) is the `anthropic-docs` skill's `references/plugins-reference.md` § Monitors — this skill builds on it, it doesn't restate it.
...
Apply to any monitor script or `monitors.json` you are about to create or edit. This host fires no skill on a file being opened, so nothing will hand you this checklist — run it yourself:

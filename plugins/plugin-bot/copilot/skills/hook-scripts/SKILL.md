---
name: hook-scripts
description: Read before creating or editing any hook script or hook registration — hooks/**/*.sh, hooks/**/*.bash, Claude Code's hooks/hooks.json, or Copilot's root hooks.json and .github/hooks/*.json. Nothing loads this skill automatically on this host, so reach for it yourself the moment one of those files comes into scope. Enforces the house hook-script contract: the per-host exit-code contracts (which are not the same contract and not universal within a host), the registration-shape and event-vocabulary deltas, portable plugin-root resolution, subdirectory-per-event layout, lib helpers, fixtures, silent-truncation traps, and BATS coverage.
---

# Hook script checklist

Apply this to the file you are about to create or edit. This host fires no skill on a file being opened, so you are reading this because you reached for it — finish it before the first edit, not after. The mistake that matters most: **treating exit 2 as a universal blocking signal.** It is not universal on either host — under Claude Code two events opt out of it entirely, and under Copilot it is a warning everywhere except two events. The second: writing state under the plugin root, which is ephemeral on plugin update.
...
6. **Plugin-owned paths resolve through the portable chain below, never dirname-walking or bare relative paths.** State, cache and logs go in the plugin *data* directory, not the plugin install. Under Claude Code that is `${CLAUDE_PLUGIN_DATA}`, which resolves to `~/.claude/plugins/data/<plugin>-<marketplace>/` (e.g. `dogfood-inline`), NOT `~/.claude/plugins/data/<plugin>/` — never write the bare-plugin-name path into a fallback or comment. Copilot's CLI reference documents `${CLAUDE_PLUGIN_DATA}` as an alias of `${COPILOT_PLUGIN_DATA}` — but that is a **substitution** fact, about tokens the host expands in a registration, and this item is about a bundled `.sh` that reads its environment. Whether Copilot **exports** either name into a hook subprocess is documented nowhere; Agent Plugins documents that export only for `${PLUGIN_ROOT}`/`${PLUGIN_DATA}`, and only for stdio MCP servers. So do not treat the data spelling as portable to a script: resolve it through a chain the same way the root is resolved, and let the namespaced `SessionStart`-exported fallback carry the case where the host sets nothing. `${PLUGIN_DATA}` is Agent-Plugins-only and is simply unset under Copilot, expanding empty in a script. See `agent-plugins-docs/references/cross-client-behavior.md` § Placeholder vocabulary.
...
### Hook sources accumulate on both hosts — a plugin's hook is never shadowed
...
| Environment change | `ConfigChange`, `CwdChanged`, `FileChanged`, `DirectoryAdded`, `WorktreeCreate`, `WorktreeRemove` | matching `##` heading; for `CwdChanged` and `FileChanged` only, also `hooks.md` § CLAUDE_ENV_FILE. Note `WorktreeCreate` sits outside the standard decision model — see rule 3 above, and `DirectoryAdded` runs in the background and cannot delay the add it reports |
| Model switch | `PreModelSwitch`, `PostModelSwitch` | matching `##` heading; also `hooks.md` § Decision-control summary for `PreModelSwitch`, which can block a switch. `PostModelSwitch` cannot. Both require Claude Code v2.1.251 or later |
...
- the `anthropic-docs` skill's `references/hooks.md` — locations and merge rules, matcher rules, the five handler types, the four-rule exit-code contract and the per-event exit-2 table, JSON output fields, timeouts and the plugin `SessionEnd` budget, `CLAUDE_ENV_FILE`, async hooks, debugging.
- the `anthropic-docs` skill's `references/hook-events.md` — per-event input/output schema, matcher values, decision-control fields.

Copilot layer:

- the `copilot-docs` skill's `references/hooks-reference.md` — the wrapped `hooks.json` shape, the 14 events and their PascalCase variants, the three handler types, matcher semantics under both spellings, output schemas, the exit-code table, the accumulate rule and the unresolved load order, cloud-agent constraints.
- the `copilot-docs` skill's `references/plugin-reference.md` — the manifest's `hooks` field and where a plugin's registration file may live.

Portable layer:

- the `agent-plugins-docs` skill's `references/agent-plugins-spec.md` — hooks are outside Agent Plugins 1.0; there is no portable registration to write.
- the `agent-plugins-docs` skill's `references/cross-client-behavior.md` — the placeholder vocabulary matrix behind the resolution chain, and which sites each token expands in.

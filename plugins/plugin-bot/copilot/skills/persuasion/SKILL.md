...
- Hook I/O contract (envelope, response, exit codes): the `anthropic-docs` skill's `references/hooks.md`.
- Per-event `hookSpecificOutput` shape for SessionStart / PostToolUse / PreToolUse: the `anthropic-docs` skill's `references/hook-events.md`.

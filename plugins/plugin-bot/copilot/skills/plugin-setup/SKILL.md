...
3. **Copy the lib templates the plugin's hook set actually needs** from `templates/hooks/lib/` into `<workspace>/hooks/lib/` verbatim, then substitute the `<PLUGIN>_` namespace prefix per each script's own header comment:
...
- **Claude Code contracts** (hook event schemas, manifest schema, the `${CLAUDE_*}` path vars, subagent frontmatter) → the `anthropic-docs` skill's `references/`.
- **Copilot contracts** (manifest fields and search order, hook events, handler types, output schemas) → the `copilot-docs` skill's `references/`.
- **Cross-host placeholder matrix** → the `agent-plugins-docs` skill's `references/cross-client-behavior.md`.

This skill scaffolds *against* those contracts — it doesn't restate them.

## What this skill is not

Not a per-event hook-writing tutorial (that's `hook-scripts` — read it yourself before any `hooks/**/*.sh`). Not a manifest field reference (that's `plugin-manifest` — read it yourself before any `plugin.json`). Not the porting procedure (that's `porting-to-copilot`). Not the platform doc mirrors (`anthropic-docs`, `copilot-docs`, `agent-plugins-docs`).
